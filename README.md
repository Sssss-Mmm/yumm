...
개인 프로젝트이고 **아직 외부 공개 출시 전**이다. 낯선 사람과 대면하는 서비스라 안전 기능이 다 서기 전에는 열지 않기로 했고, 그 기준을 M4로 정의했다. **M4는 구현이 끝났고 출시 전 검증에서 나온 결함을 정리하는 중이다.**
...
| M4 | 신고·차단·성인 인증·약관 동의·안전 수칙, 2인 폴백, 이메일 인증, 탈퇴 익명화, 관리자 신고 처리 | 구현 완료, 검증 중 — 탈퇴 후 토큰 무효화와 가입 입력 검증을 마저 막는 중 |
| M5 | 1개 지역 클로즈드 베타 | 다음 |
