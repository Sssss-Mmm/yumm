...
| 테스트 | 무엇을 잡나 |
|---|---|
| `GroupMatcherTest` | 그리디 편성, 성별 쌍방 조건, 최소/최대 인원, 2인 폴백(쌍방 옵트인·만료창) |
...
| `MatchAllowPairWiringTest` | 2인 옵트인이 요청 본문에서 편성 후보까지 도달하는지 |
...
개인 프로젝트이고 **아직 외부 공개 출시 전**이다. 낯선 사람과 대면하는 서비스라 안전 기능이 다 서기 전에는 열지 않기로 했고, 그 기준을 M4로 정의했다. **M4까지 닫혔다** — 출시 전 검증에서 나온 결함(탈퇴 계정 인증 우회, 가입 입력 검증 부재)을 고치고 재검증까지 마쳤다.
...
| M3 | 매칭 성사·해체 알림, 식사 당일 아침 리마인드 | 완료 — SMTP로 3종 발송. 리마인드 중복은 `reminded_at` 컬럼 하나로 막는다 |
| M4 | 신고·차단·성인 인증·약관 동의·안전 수칙, 2인 폴백, 이메일 인증, 탈퇴 익명화, 관리자 신고 처리 | 완료 — **출시 가능 상태** |
| M5 | 1개 지역 클로즈드 베타 | 다음 |

M4를 닫으면서 정리한 것 몇 가지:

- **2인 폴백**은 유동성 문제(같은 지역·시간대에 3명이 30분 안에 겹쳐야 한다)에 대한 대비다. 옵트인 + 쌍방 + 만료 임박 시에만 적용하고, 노쇼에 가장 취약한 조합이라 식사 당일 리마인드가 붙기 전에는 켜지 않기로 했었다. 리마인드가 들어오면서 그 조건이 충족됐다.
- **탈퇴**는 하드 삭제였는데, 신고·차단·채팅 기록이 전부 `nullable=false`로 사용자를 참조해서 기록이 있는 사람은 탈퇴 시 FK 위반으로 실패했다. 익명화로 바꿔 개인정보는 지우고 신고 이력은 남긴다.
- **이탈자 채팅 구독**은 프레임 시점에만 검사해서, 그룹을 나가도 이미 열린 구독으로 대화를 계속 받을 수 있었다. 이탈 시점에 해당 방 구독만 골라 끊는다.

가장 큰 남은 과제는 **유동성**이다. 3~4인을 고집하는 이상 같은 지역·날짜·시간대에 최소 3명이 모여야 하고, 이게 이 서비스의 진짜 리스크다. 지역을 1~2곳으로 좁혀 여는 것과 2인 폴백이 그에 대한 대비다. 폴백은 구현했지만 **옵트인 체크박스 노출은 M5 진입 시 리마인드 알림 여부에 달려 있다** — 2인은 상대가 안 나오면 만남이 0명이라 리마인드가 선행 조건이다(`requirements.md` FR-G-11).
